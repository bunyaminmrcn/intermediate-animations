        ...
        <>
            <Text
                style={{
                    ...
                    display: item._i == index ? 'flex': 'none'
                }}
                numberOfLines={1}
            ...
            <Text style={{ fontSize: 12, opacity: 0.4,display: item._i == index ? 'flex': 'none' }}>{item.subtitle}</Text>
...
export default () => {
    ...
    const progress = Animated.modulo(Animated.divide(scroolX, width), width);
    const [pageIndex, setIndex] = React.useState(0)
            ...
            <SafeAreaView style={{ marginTop: SPACING * 4 }}>
                <View style={{ height: IMAGE_HEIGHT * 2.1 }}>
                    <Animated.FlatList
                        ...
                        horizontal
                        pagingEnabled
                        onScroll={Animated.event([{ nativeEvent: { contentOffset: { x: scroolX } } }], {
                        ...
                        onMomentumScrollEnd={ev => {
                            const _index = Math.floor(ev.nativeEvent.contentOffset.x / width)
                            setIndex(_index)
                            console.log(_index)
                        }}
                        ...
                        renderItem={({ item, index }) => {
                            ...
                            const translateY = scroolX.interpolate({
                                inputRange,
                                outputRange: [50, 1, 20]
                            })
                    ...
                    >
                        {
                            DATA.map((item, index) => {
                                ...
                                */
                                /*
                                 const rotateY = scroolX.interpolate({
                                     inputRange,
                                ...
                                >
                                    <Content item={item} index={pageIndex}/>
                                </Animated.View>
                            })
                        }
                    </View>
                ...
                >
                    <TouchableOpacity
                        ...
                        style={{ opacity: pageIndex === 0 ? 0.2 : 1 }}
                        onPress={() => {
                    ...
                    <TouchableOpacity
                        ...
                        style={{ opacity: pageIndex === DATA.length - 1 ? 0.2 : 1 }}
                        onPress={() => {
